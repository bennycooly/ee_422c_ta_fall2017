...
class Grader:
    ...
    def compile_sources(self):
        source_filenames = self.config["source"]["files"]
        for student in self.student_map.values():
            ...
            for filename in source_filenames:
                matched_files = glob.glob(os.path.join(source_path, "**", filename), recursive=True)
    ...
    def run_program(self):
        package = self.config["source"]["package"]
        for student in self.student_map.values():
            print("Running program for student: " + student["sis_user_id"])
            main_class = self.config["source"]["main"]
            source_dir = os.path.join(self.dirs["students"], student["sis_user_id"], "submission")
            classpath_matches = glob.glob(os.path.join(source_dir, "**", package), recursive=True)
            if len(classpath_matches) == 0:
                continue
            classpath = os.path.join(classpath_matches[0], "..")
            # print(classpath)
            completed = subprocess.run(["java", "-cp", classpath, package + '.' + main_class], input=str.encode("3\n10321424893298"))
